...
let problemsContainer = document.getElementById("inputs_container");
let radios = document.querySelectorAll('input[name="issue_type"]');
let issueSummary = document.getElementById("summary_container");
let problemSummary = document.getElementById("problem_summary");
...
async function submitIssue(){
  ...
  pcStatus = radiovalue == null ? "working" : "not working"
  let pcData = {
    pcId: pcIdInput,
    pcStatus: pcStatus,
    comment: problemSummary.value,
  };

  const csrfToken = getCookie("csrftoken");
  await fetch("http://127.0.0.1:8000/update/pc", {
    method: "POST",
